...
from gnowsys_ndf.settings import GSTUDIO_ELASTICSEARCH, OER
        ...
        if GSTUDIO_SITE_NAME == "clix":
            ...
            if request.META['QUERY_STRING']  == "True":
                ...
                '''if GSTUDIO_ELASTICSEARCH:
                    q = Q("match", type="Group")&Q("match", name="Mathematics")
                    s = Search(index = 'nodes').using(client).query(q)
                    response1 = s.execute()
                    domain1 = response1
                    for hit in response1:
                        print hit.id
                    context_variable.update({'modules_1':modules1})
                    q = Q("match", type="Group")&Q("match", name="Science")
                    s = Search(index = 'nodes').using(client).query(q)
                    response2 = s.execute()
                    domain2 = response2
                    for hit in response2:
                        print hit.id
                    context_variable.update({'modules_2':modules2})
                    q = Q("match", type="Group")&Q("match", name="English")
                    s = Search(index = 'nodes').using(client).query(q)
                    response3 = s.execute()
                    domain3 = response3
                    for hit in response3:
                        print hit.id
                    context_variable.update({'modules_3':modules3})
                    #print response.count()'''
                    #print "/////////////////////////IN HOME //////////////////////////"
                title = 'OER'
                context_variable.update({'title':title})
                return render_to_response(
                                        GSTUDIO_SITE_LANDING_TEMPLATE,
                                        context_variable,
                                        context_instance=RequestContext(request)
