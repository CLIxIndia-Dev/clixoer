    ...
    def save_to_es(django_document):
        try:
            ...
            if django_document["type"] == "GSystem":
                ...
                print index
                print "hey2"
                es.index(index=index, doc_type="nodes", id=django_document["id"], body=django_document)
                #file_name.write(document["id"] + '\n')
                '''if django_document["type"]=="GSystem":
                    print("hey buddy1")
                    if('if_file' in django_document.keys()):
                        if(django_document["if_file"]["mime_type"] is not None):
                            ...
                            doc_type = data[0]
                            print("no no wrong place")
                        else:
                            print("may be true god knows")
                            doc_type = "notmedia"
                    else:
                        print("pls be right")
                        doc_type = "notmedia"
                    ...
                    print("i also don't care buddy")
                    doc_type = "dontcare"'''

                '''if (not es.indices.exists("gsystem")):
                    print("so here we are out of if phewww")
                    print "request:",request_body,'\n',django_document
                    #res = es.indices.create(index="gsystem", body=request_body)'''
                print("pheww out agAIN")
            ...
            es.index(index=index, doc_type=index.lower(), id=django_document["id"], body=django_document)
